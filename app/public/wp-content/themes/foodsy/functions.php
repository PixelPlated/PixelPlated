<?php
	...
	function foodsy_after_setup_theme()
	{
		load_theme_textdomain('foodsy', get_template_directory() . '/languages');
		register_nav_menus(array('foodsy_theme_menu_location' => esc_html__('Theme Navigation Menu', 'foodsy')));
	...
	add_action('after_setup_theme', 'foodsy_after_setup_theme');


/* ============================================================================================================================================= */


	function foodsy_default_fonts($font = "")
	{
		if     ($font == 'text_logo')      { $font = 'FONT_LOCAL_Arkipelago'; }
		elseif ($font == 'menu')           { $font = 'Jost'; }
		elseif ($font == 'widget_title')   { $font = 'FONT_LOCAL_TeXGyreAdventor'; }
		elseif ($font == 'h1')             { $font = 'DM Serif Display'; }
		elseif ($font == 'h2_h6')          { $font = 'Jost'; }
		elseif ($font == 'slider_title')   { $font = 'FONT_LOCAL_Now'; }
		elseif ($font == 'body')           { $font = 'DM Sans'; }
...
/* ============================================================================================================================================= */
	...
	/*
		...
		simply create your own foodsy_theme_comments(), and that function will be used instead.
		
		Used as a callback by wp_list_comments() for displaying the comments.
	*/
	
	function foodsy_theme_comments($comment, $args, $depth)
	{
			...
			case 'trackback' :
				...
				?>
					<li id="comment-<?php comment_ID(); ?>" <?php comment_class(); ?>>
						<p>
							<?php
								esc_html_e('Pingback:', 'foodsy');
							?>
							<?php
								comment_author_link();
							?>
							<?php
								edit_comment_link(esc_html__('(Edit)', 'foodsy'), '<span class="edit-link">', '</span>');
							?>
						</p>
				<?php
						...
						<article id="comment-<?php comment_ID(); ?>" class="comment">
							<header class="comment-meta comment-author vcard">
								...
								<span class="comment-date">
									...
									?>
									<?php
										esc_html_e('at', 'foodsy');
									?>
									<?php
										echo get_comment_time();
									?>
									<?php
										edit_comment_link(esc_html__('Edit', 'foodsy'), '<span class="comment-edit-link">', '</span>');
									?>
								</span>
							</header>
									...
									if ('0' == $comment->comment_approved)
									{
										?>
											<p class="comment-awaiting-moderation">
												<?php
													esc_html_e('Your comment is awaiting moderation.', 'foodsy');
												?>
											</p>
										<?php
									}
							...
							<div class="reply">
								<?php
									comment_reply_link(array_merge($args,
																   array('reply_text' => esc_html__('Reply', 'foodsy'),
																		 'after'      => ' ' . '<span>' . esc_html__('&#8595;', 'foodsy') . '</span>',
																		 'depth'      => $depth,
...
/* ============================================================================================================================================= */
	...
	function foodsy_post_tags()
	{
		$tags = get_theme_mod('foodsy_setting_tags', 'Yes');
...
/* ============================================================================================================================================= */
	...
	if (! function_exists('foodsy_archive_layout'))
	{
		function foodsy_archive_layout()
		{
			$layout = 'Regular';
			
			if (is_home() || is_tax('post_format'))
			{
				$layout = get_theme_mod('foodsy_setting_layout_blog', 'List');
			}
			elseif (is_tax())
			{
			...
			elseif (is_category())
			{
				$layout = get_theme_mod('foodsy_setting_layout_category', 'List');
			}
			elseif (is_tag())
			{
				$layout = get_theme_mod('foodsy_setting_layout_tag', 'List');
			}
			elseif (is_author())
			{
				$layout = get_theme_mod('foodsy_setting_layout_author', 'List');
			}
			elseif (is_date())
			{
				$layout = get_theme_mod('foodsy_setting_layout_date', 'List');
			}
			elseif (is_search())
			{
				$layout = get_theme_mod('foodsy_setting_layout_search', 'Grid');
			}
			elseif (is_archive())
			{
...
/* ============================================================================================================================================= */
	...
	function foodsy_1st_full_yes_no()
	{
		$first_full = 'No';
...
/* ============================================================================================================================================= */
	...
	function foodsy_blog_grid_post_width()
	{
		$grid_post_width = get_theme_mod('foodsy_setting_grid_post_width', '340');
		
		echo esc_attr($grid_post_width);
	}
	
	
	function foodsy_blog_grid_type()
	{
		$grid_type = 'masonry';
		...
		{
			$grid_type = get_theme_mod('foodsy_setting_grid_type', 'masonry');
		}
...
/* ============================================================================================================================================= */
	...
	function foodsy_wp_head__theme_directory_url()
	{
		// Used for local_font_url in customizer.
		
		if (is_customize_preview())
		{
			$theme_directory_url = get_template_directory_uri(); // http://www.example.com/wp-content/themes/foodsy
			...
			?>
				...
				<meta id="foodsy_theme_directory_url" name="foodsy_theme_directory_url" content="<?php echo esc_url($theme_directory_url); ?>">

			<?php
		}
	}
	
	add_action('wp_head', 'foodsy_wp_head__theme_directory_url');
...
/* ============================================================================================================================================= */
	...
	*/
	...
	function foodsy_theme_post_format_chat_content( $content )
	{
		...
		foreach ( $chat_rows as $chat_row )
		{
			...
			if ( strpos( $chat_row, $separator ) )
			{
				...
				$speaker_id = foodsy_theme_post_format_chat_row_id( $chat_author );
	...
	*/
	...
	function foodsy_theme_post_format_chat_row_id( $chat_author )
	{
		global $_post_format_chat_ids;
	...
	add_filter( 'the_content', 'foodsy_theme_post_format_chat_content' );


/* ============================================================================================================================================= */


	function foodsy_blog_page_link_html($url)
	{
		?>
			<div class="section-launch">
				<a class="button" href="<?php echo esc_url($url); ?>">
					<?php
						esc_html_e('See All Posts', 'foodsy');
					?>
				</a> <!-- .button -->
			</div> <!-- .section-launch -->
		<?php
	...
	function foodsy_blog_page_link()
	{
		...
		{
			$home_url = home_url('/');
			foodsy_blog_page_link_html($home_url);
		}
		else
		{
			...
			if ($blog_page_id)
			{
				...
				foodsy_blog_page_link_html($blog_page_url);
			}
		}
	}
