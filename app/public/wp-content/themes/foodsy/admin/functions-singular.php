<?php
	...
	function foodsy_featured_image($post_header_top, $parallax, $overlay)
	{
		if ($parallax)
		{
			?>
				<div class="post-thumbnail" style="background-image: url(<?php the_post_thumbnail_url('foodsy_image_size_2'); ?>);" data-medium-image="<?php the_post_thumbnail_url('foodsy_image_size_1'); ?>" data-large-image="<?php the_post_thumbnail_url('foodsy_image_size_7'); ?>">
					<div class="post-wrap">
						<?php
							if ($overlay)
							{
	...
	function foodsy_featured_video($browser_address_url, $parallax, $overlay)
	{
		if ($parallax)
		{
			?>
	...
	function foodsy_featured_media($post_header_top, $post_header_inline, $featured_image_position, $where = "", $parallax = "", $overlay = "")
	{
		...
		if (($where == $featured_image_position) || ($post_header_inline == 'is-top-content-single-medium with-title-full'))
		{
				...
				($post_header_inline == 'post-header-classic is-featured-image-right')) // Force for Image Left and Image Right.
			{
				if (has_post_thumbnail())
				{
					foodsy_featured_image($post_header_top, $parallax, $overlay);
				}
				elseif (! empty($browser_address_url))
				{
					foodsy_featured_video($browser_address_url, $parallax, $overlay);
				}
			}
			elseif (! empty($browser_address_url))
			{
	...
	function foodsy_post_header__content($post_header_top, $post_header_inline, $featured_image_position, $parallax, $overlay, $only_title, $only_media)
	{
		...
		$category_link_style = foodsy_category_link_style();
	...
	function foodsy_post_header__inline($post_header_top, $post_header_inline, $featured_image_position, $parallax, $overlay, $only_title, $only_media)
	{
		foodsy_post_header__content(
			$post_header_top,
			$post_header_inline,
			$featured_image_position,
			$parallax,
	...
	function foodsy_post_header__top($post_header_top, $post_header_inline, $featured_image_position, $parallax, $overlay, $only_title, $only_media)
	{
		?>
	...
	function foodsy_post_header__inline_chooser($post_style, $post_header_top, $featured_image_position, $only_title, $only_media)
	{
		switch ($post_style)
		{
	...
	function foodsy_post_header__top_chooser($post_style, $post_header_top, $featured_image_position, $only_title, $only_media)
	{
		switch ($post_style)
		{
	...
	function foodsy_post_header($post_header_top)
	{
		$featured_image_position = get_theme_mod('foodsy_setting_post_featured_image_position', 'below_title');
		$post_style              = foodsy_core_post_style();
		
		if (($post_style == 'inherit') || ($post_style == null) || ($post_style == false))
		{
			if (is_singular('page'))
			{
				$post_style = get_theme_mod('foodsy_setting_page_style_global', 'post-header-classic'); // Get page style class. (customizer) --- PAGE
			}
			elseif (is_singular('post'))
			{
				$post_style = get_theme_mod('foodsy_setting_post_style_global', 'is-top-content-single-full with-parallax'); // Get post style class. (customizer) --- POST
			}
			else
			{
				$post_style = get_theme_mod('foodsy_setting_custom_post_style_global', 'is-top-content-single-full with-overlay'); // Get post style class. (customizer) --- CUSTOM POST TYPES
			}
		}
		...
		if ($post_header_top && ($post_style == 'is-top-content-single-medium with-title-full')) // Title Full. (top header)
		{
			$only_title = true;
			$only_media = false;
			foodsy_post_header__top_chooser(
				$post_style,
				$post_header_top,
				$featured_image_position,
				$only_title,
		...
		elseif ((! $post_header_top) && ($post_style == 'is-top-content-single-medium with-title-full')) // Title Full. (inline header)
		{
			$only_title = false;
			$only_media = true;
			foodsy_post_header__inline_chooser(
				$post_style,
				$post_header_top,
				$featured_image_position,
				$only_title,
