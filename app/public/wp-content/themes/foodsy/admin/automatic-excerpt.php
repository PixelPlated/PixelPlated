<?php
	...
	function foodsy_excerpt_length($length)
	{
		$layout = 'Regular';
		...
		{
			return get_theme_mod('foodsy_setting_excerpt_length', '65');
		}
		else
		{
			return get_theme_mod('foodsy_setting_excerpt_length_layout_grid', '35');
		}
	}
	
	add_filter('excerpt_length', 'foodsy_excerpt_length', 999);


/* ============================================================================================================================================= */


	function foodsy_read_more_link()
	{
		$more_link_text = get_theme_mod('foodsy_setting_more_link_text', "");
		$more_link_text = trim($more_link_text);
		
		if (empty($more_link_text))
		{
			$more_link_text = esc_html__('Read More', 'foodsy');
		}
		
		return '<p class="more"><a class="more-link" href="' . esc_url(get_permalink()) . '">' . $more_link_text . '</a></p>';
	}
	
	add_filter('the_content_more_link', 'foodsy_read_more_link');
	
	
	function foodsy_excerpt_more($more)
	{
		return '... ' . foodsy_read_more_link();
	}
	
	add_filter('excerpt_more', 'foodsy_excerpt_more');


/* ============================================================================================================================================= */


	function foodsy_content()
	{
		...
		else
		{
			...
			if (! has_post_format('chat'))
			{
				if (has_excerpt())
				{
					...
					echo foodsy_read_more_link();
				}
				else
				{
					$automatic_excerpt = get_theme_mod('foodsy_setting_automatic_excerpt', 'standard'); // Regular archives.
		...
		wp_link_pages(
			array(
				'before'      => '<div class="page-links"><span class="page-links-title">' . esc_html__('Pages:', 'foodsy') . '</span>',
				'after'       => '</div>',
