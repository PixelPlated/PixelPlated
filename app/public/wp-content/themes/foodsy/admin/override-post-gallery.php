<?php
	...
	function foodsy_gallery_type__slider($atts)
	{
		...
		if ($items_with_commas != "")
		{
			...
			$output .= '<div class="owl-carousel" data-items="1" data-loop="true" data-center="false" data-mouse-drag="true" data-nav="true" data-dots="true" data-autoplay="false" data-autoplay-speed="600" data-autoplay-timeout="2000">';
				...
				{
					$size = 'foodsy_image_size_7';
				}
				else
				{
					$size = 'foodsy_image_size_1';
				}
	...
	function foodsy_gallery_type__grid($atts)
	{
		extract(
			shortcode_atts(
				array(
		...
		if ($items_with_commas != "")
		{
			...
			$output .= '<div class="gallery ' . (($link == "") ? 'link-to-attachment-page' : 'link-to-' . $link) . '">';
				...
				{
					$image_big = "";
					$image_big_width_cropped = wp_get_attachment_image_src($item, 'foodsy_image_size_7'); // magnific-popup-width
					...
					{
						$image_big_height_cropped = wp_get_attachment_image_src($item, 'foodsy_image_size_8'); // magnific-popup-height
					...
					if ($size == 'full')
					{
						$image_small = wp_get_attachment_image_src($item, 'foodsy_image_size_1'); // gallery-type-grid
					}
					else
					{
						$image_small = wp_get_attachment_image_src($item, 'foodsy_image_size_6'); // gallery-type-grid
					}
	...
	function foodsy_portfolio_page__lightbox_gallery($atts)
	{
		extract(shortcode_atts(array('ids'     => "",
									 'orderby' => "",
									 'link'    => "",
		...
		if ($items_with_commas != "")
		{
			...
			if ($orderby == 'rand')
			{
				...
				$feat_img = $foodsy_portfolio_item_has_feat_img;
				...
				foreach ($items_in_array as $item)
				{
					...
					if ($first_item)
					{
						if ($feat_img)
						{
							$output .= '<a class="lightbox" ' . $image_caption . ' href="' . esc_url($image_big) . '">' . foodsy_portfolio_item_feat_img__lightbox_gallery($foodsy_portfolio_page_grid_type__lightbox_gallery) . '</a>';
						}
						else
						{
	...
	function foodsy_post_gallery($output = "", $atts = null, $content = false, $tag = false)
	{
		$new_output = $output;
		
		if ((is_page_template('page_template-portfolio.php') || is_tax('portfolio-category')) && has_post_format('gallery'))
		{
			$new_output = foodsy_portfolio_page__lightbox_gallery($atts);
		}
		else
		{
			$gallery_type = foodsy_core_gallery_type();
			
			if ($gallery_type == 'slider')
			{
				$new_output = foodsy_gallery_type__slider($atts);
			}
			else
			{
				$new_output = foodsy_gallery_type__grid($atts);
			}
		}
	...
	add_filter('post_gallery', 'foodsy_post_gallery', 10, 4);
