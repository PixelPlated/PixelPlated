<?php
	...
	if (! function_exists('foodsy_single_navigation_html'))
	{
		function foodsy_single_navigation_html()
		{
						...
						<div class="nav-previous">
							<div class="nav-desc">
								<?php
									previous_image_link(
										false,
										'<span class="meta-nav">&#8592;</span>' . esc_html__('Previous Image', 'foodsy')
									);
								?>
							</div> <!-- .nav-desc -->
						</div> <!-- .nav-previous -->
						<div class="nav-next">
							<div class="nav-desc">
								<?php
									next_image_link(
										false,
										esc_html__('Next Image', 'foodsy') . '<span class="meta-nav">&#8594;</span>'
									);
								?>
							</div> <!-- .nav-desc -->
						</div> <!-- .nav-next -->
									...
									?>
										<div class="nav-previous">
											<?php
												...
												{
													$feat_img     = wp_get_attachment_image_src(get_post_thumbnail_id($previous_post->ID), 'foodsy_image_size_5');
												...
												previous_post_link(
													'<div class="nav-desc"><h4>' . esc_html__('Previous Post', 'foodsy') . '</h4>%link</div>',
									...
									?>
										<div class="nav-next">
											<?php
												...
												{
													$feat_img     = wp_get_attachment_image_src(get_post_thumbnail_id($next_post->ID), 'foodsy_image_size_5');
												...
												next_post_link(
													'<div class="nav-desc"><h4>' . esc_html__('Next Post', 'foodsy') . '</h4>%link</div>',
	...
	if (! function_exists('foodsy_single_navigation'))
	{
		function foodsy_single_navigation()
		{
			$post_navigation = get_theme_mod('foodsy_setting_post_navigation', 'Yes');
			
			if ($post_navigation != 'No')
			{
				foodsy_single_navigation_html();
			}
		}
	}
