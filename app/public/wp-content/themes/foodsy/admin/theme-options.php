<?php
	...
	function foodsy_create_tabs($current = 'customizer')
	{
		$tabs = array(
			...
			<h2 class="nav-tab-wrapper">
				<?php
					foreach ($tabs as $tab => $name)
					{
						...
						echo "<a class='nav-tab$class' href='?page=foodsy-theme-options&tab=$tab'>$name</a>";
					}
				?>
			</h2>
		<?php
...
/* ============================================================================================================================================ */
	...
	function foodsy_theme_options_page()
	{
		global $pagenow;
		...
		?>
			<div class="wrap wrap2">
				...
				<?php
					if ( isset( $_GET['tab'] ) )
					{
						foodsy_create_tabs($_GET['tab']);
					}	
					else
					{
						foodsy_create_tabs('customizer');
					}
				?>
				...
				<div id="poststuff">
					<?php
						// theme options page
						if ( $pagenow == 'themes.php' && $_GET['page'] == 'foodsy-theme-options' )
						{
							// tab from url
							if ( isset( $_GET['tab'] ) )
							{
							...
							switch ($tab)
							{
								case 'customizer' :
									...
									?>
										<div class="postbox">
											<div class="inside">
												<?php
													$foodsy_admin_url = admin_url( 'themes.php?page=foodsy-theme-options' );
												?>
												<form method="post" class="ajax-form" action="<?php echo esc_url( $foodsy_admin_url ); ?>">
													<?php
								...
								case 'widget-areas' :
									...
									if (isset($_GET['saved']) == 'true')
									{
										$foodsy_no_sidebar_name = get_option('foodsy_no_sidebar_name');
										
										if ($foodsy_no_sidebar_name == "")
										{
									...
									{
										delete_option('foodsy_sidebars_with_commas');
									...
									?>
										<div class="postbox">
											<div class="inside">
												<?php
													$foodsy_admin_url = admin_url('themes.php?page=foodsy-theme-options&tab=widget-areas');
												?>
												<form method="post" action="<?php echo esc_url($foodsy_admin_url); ?>">
													<?php
														wp_nonce_field( "settings-page" );
													?>
													<table>
														<tr>
															<td class="option-left">
																...
																<input type="text" name="foodsy_new_sidebar_name" required="required" style="width: 100%;" value="">
															</td>
															<td class="option-right">
														...
														<tr>
															<td class="option-left">
																<?php
																	$foodsy_admin_url = admin_url( 'themes.php?page=foodsy-theme-options&tab=widget-areas&deleted=true' );
																?>
																<a class="button button-large" style="margin-top: 20px;" href="<?php echo esc_url( $foodsy_admin_url ); ?>">Delete</a>
															</td>
															<td class="option-right">
																Remove widget areas.
															</td>
...
/* ============================================================================================================================================ */
	...
	function foodsy_theme_save_settings()
	{
		global $pagenow;
		...
		if ( $pagenow == 'themes.php' && $_GET['page'] == 'foodsy-theme-options' )
		{
			...
			switch ( $tab )
			{
				...
				case 'widget-areas' :
					...
					update_option( 'foodsy_no_sidebar_name', esc_attr( $_POST['foodsy_new_sidebar_name'] ) );
					
					if ( esc_attr( $_POST['foodsy_new_sidebar_name'] ) != "" )
					{
						$foodsy_sidebars_with_commas = get_option( 'foodsy_sidebars_with_commas', "" );
						
						if ( $foodsy_sidebars_with_commas == "" )
						{
							update_option( 'foodsy_sidebars_with_commas', esc_attr( $_POST['foodsy_new_sidebar_name'] ) );
						}
						else
						{
							update_option( 'foodsy_sidebars_with_commas', get_option( 'foodsy_sidebars_with_commas' ) . ',' . esc_attr( $_POST['foodsy_new_sidebar_name'] ) );
						}
					}
...
/* ============================================================================================================================================ */
	...
	function foodsy_load_settings_page()
	{
		if ( isset( $_POST["settings-submit"] ) == 'Y' )
		{
			check_admin_referer( "settings-page" );
			foodsy_theme_save_settings();
			$url_parameters = isset( $_GET['tab'] ) ? 'tab=' . $_GET['tab'] . '&saved=true' : 'saved=true';
			wp_redirect( admin_url( 'themes.php?page=foodsy-theme-options&' . $url_parameters ) );
			exit;
		}
	}
...
/* ============================================================================================================================================ */
	...
	function foodsy_theme_menu()
	{
		$settings_page = add_theme_page('Theme Options',
										'Theme Options',
										'edit_theme_options',
										'foodsy-theme-options',
										'foodsy_theme_options_page' );
		
		add_action( "load-{$settings_page}", 'foodsy_load_settings_page' );
	}
	
	add_action( 'admin_menu', 'foodsy_theme_menu' );
